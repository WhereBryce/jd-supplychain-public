...
| RDC 库存查询 | 工具·加密访问 | 密码直入，支持京东码搜索、商品名称中文模糊搜索与 RDC 筛选；仅保留 7 个必要字段 | <https://wherebryce.github.io/jd-supplychain-public/pages/rdc-inventory-query.html> |
...
jd-supplychain-public/          # Pages 从 /(root) 发布
  ...
  data/
    ...
    rdc-product-search/             # 64 个中文/编码字符搜索分片
  scripts/
...
5. 按 SHA-256(SKU) 拆成 64 个库存分片；商品目录另拆为极小清单和 64 个字符搜索分片。
6. 使用 PBKDF2-SHA256（600,000 次）从访问密码派生密钥。
7. 使用 AES-256-GCM 加密完整库存、商品搜索索引和查询分片，只提交密文。
8. 浏览器输入密码后在本地解密；密码不会上传或写入仓库。

输入密码后只下载约 1 KB 的加密搜索清单。京东码搜索直接定位库存分片；中文商品名模糊搜索只下载一个字符搜索分片，展示候选商品后再按需加载对应库存分片。结果可继续按 RDC 筛选，并通过 ETag 复用未变化的本地缓存。
