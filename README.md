...
| RDC 库存查询 | 工具·加密访问 | 密码在浏览器本地解密 AES-GCM 密文，支持 SKU、商品、RDC、品牌、状态查询及分页汇总 | <https://wherebryce.github.io/jd-supplychain-public/pages/rdc-inventory-query.html> |
...
4. 使用 PBKDF2-SHA256（600,000 次）从访问密码派生密钥。
5. 使用 AES-256-GCM 加密完整库存 JSON，只提交 `data/rdc-inventory.enc.json` 密文。
6. 浏览器输入密码后在本地解密；密码不会上传或写入仓库。

浏览器会显示密文下载进度，并通过 ETag 复用未变化的本地缓存；首次下载完成后，重复访问无需再次下载整份库存。
